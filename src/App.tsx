...
export default function App() {
  ...
  const saveClient = async (e: React.FormEvent) => {
    ...
    let prevName = null;
    if (editingClient && editingClient.name !== newName) prevName = editingClient.name;
    ...
    if (!error) {
      setIsModalOpen(false); setIsRenewModalOpen(false); setEditingClient(null);
      fetchClients();
    }
  };
    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm space-y-10 text-white">
        <div className="text-center space-y-4">
          <div className="w-20 h-20 bg-amber-500 rounded-[2rem] flex items-center justify-center mx-auto shadow-lg"><Scissors className="text-black w-10 h-10" /></div>
          <h1 className="text-4xl font-black italic tracking-tighter uppercase tracking-widest">BARBER <span className="text-amber-500">PRO</span></h1>
        </div>
        <form onSubmit={async (e) => { e.preventDefault(); const {error} = await supabase.auth.signInWithPassword({email, password}); if(error) setLoginError('Erro no login'); }} className="bg-zinc-900/50 p-8 rounded-[2rem] border border-white/5 space-y-4 shadow-2xl">
          ...
          <button className="w-full bg-amber-500 text-black py-4 rounded-2xl font-black uppercase active:scale-95 transition-all">Entrar</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-amber-500/30">
      ...
      <header className="sticky top-0 z-40 px-6 h-24 flex items-center justify-between bg-[#050505]/90 backdrop-blur-2xl border-b border-white/5">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsProfileModalOpen(true)} className="w-12 h-12 bg-zinc-900 border border-white/10 rounded-2xl flex items-center justify-center text-zinc-400 hover:text-amber-500"><UserIcon size={20} /></button>
          <div>
            <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest leading-none mb-1">{profTitle}: {userName || '...'}</p>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setIsHistoryView(!isHistoryView)} className={cn("p-3 rounded-xl transition-all", isHistoryView ? "bg-amber-500 text-black" : "bg-zinc-900 text-zinc-500")}><FolderClock size={20} /></button>
          <button onClick={() => { setEditingClient(null); setIsModalOpen(true); }} className="bg-amber-500 text-black p-3 rounded-xl active:scale-95 transition-all"><UserPlus size={20} /></button>
          <button onClick={() => supabase.auth.signOut()} className="p-3 text-zinc-700 hover:text-red-500"><LogOut size={20} /></button>
        </div>
      </header>
      ...
      <main className="p-6 max-w-2xl mx-auto space-y-8 pb-32">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1 group">
            ...
            <input type="text" placeholder="Localizar cliente..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-zinc-900/50 border border-white/5 rounded-2xl pl-14 pr-6 py-4 outline-none focus:border-amber-500/30 text-sm font-medium text-white" />
          </div>
          <div className="relative">
            <select value={filterPeriod} onChange={(e) => setFilterPeriod(e.target.value)} className="appearance-none bg-zinc-900 border border-white/10 rounded-2xl px-6 py-4 pr-12 text-[10px] font-black uppercase tracking-widest outline-none focus:border-amber-500/50 w-full sm:w-auto text-white cursor-pointer">
              <option value="today">Hoje</option>
              <option value="week">7 Dias</option>
              <option value="month">30 Dias</option>
              <option value="all">Todo Período</option>
            </select>
            <ChevronDown size={14} className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-500" />
          </div>
        </div>

        <div className="space-y-4">
          <div className="flex items-center justify-between px-2 text-zinc-600">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em]">{isHistoryView ? 'Histórico' : 'Atividade'}</h3>
            <button onClick={() => setIsSelectionMode(!isSelectionMode)} className="text-[9px] font-black uppercase tracking-widest hover:text-amber-500">{isSelectionMode ? "Concluir" : "Excluir Vários"}</button>
          </div>
          ...
          {(isHistoryView ? historyList : dashboardList)
            ...
            .map(c => {
              ...
              return (
                <div key={c.id} className={cn("bg-zinc-900/30 border rounded-[2.5rem] p-5 flex items-center justify-between transition-all", isSel ? "border-amber-500 bg-amber-500/5 shadow-[0_0_20px_rgba(217,119,6,0.1)]" : "border-white/5")}>
                  <div className="flex items-center gap-4">
                    {isSelectionMode ? (
                      ...
                      <div className="relative">
                        ...
                        <div className={cn("absolute -top-2 -right-2 flex items-center gap-0.5 bg-black px-2 py-0.5 rounded-full border border-white/10 text-[10px] font-black", getStreakColor(streak))}><Flame size={10} className="fill-current" /> {streak}</div>
                      </div>
                    )}
                    <div>
                      <p className="font-black text-base text-white/90 leading-none">{c.name} {c.previous_name && <span className="text-[9px] text-zinc-600 block italic">(anteriormente {c.previous_name})</span>}</p>
                      <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-tighter mt-1.5">R$ {c.price} • {c.services?.join(' + ')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <a href={`https://wa.me/55${c.phone}`} target="_blank" className="p-3 bg-amber-500 text-black rounded-xl shadow-lg"><MessageSquare size={16} /></a>
                      <button onClick={() => { setPhonesToDelete([c.phone]); setIsDeleteModalOpen(true); }} className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500/20"><Trash2 size={16} /></button>
                    </div>
                  </div>
                </div>
              );
        ...
        {isSelectionMode && selectedPhones.length > 0 && (
          <div className="fixed bottom-8 left-0 right-0 px-6 z-50 flex justify-center">
            <button onClick={() => { setPhonesToDelete(selectedPhones); setIsDeleteModalOpen(true); }} className="max-w-md w-full bg-red-600 text-white py-5 rounded-[2rem] font-black uppercase tracking-widest text-[10px] shadow-2xl animate-bounce">Apagar {selectedPhones.length} Clientes</button>
          </div>
        )}
      </main>

      {/* MODAIS EMBUTIDOS (CUSTOMIZADOS) */}
      {isRenewModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6 backdrop-blur-2xl">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-md rounded-[3rem] p-10 shadow-3xl text-white">
             ...
             <p className="text-zinc-500 text-[10px] text-center mb-8 uppercase font-bold tracking-widest">Confirme os dados para o novo registro</p>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-[60] flex items-center justify-center p-6 backdrop-blur-2xl">
          <div className="bg-zinc-900 border border-red-500/20 w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl text-white">
             ...
             <p className="text-zinc-500 text-[10px] mb-8 uppercase font-bold tracking-widest leading-relaxed">Você está apagando o cliente e TODO o histórico dele.<br/><span className="text-red-500/50 font-black">Não pode ser desfeita.</span></p>
             <div className="flex flex-col gap-3">
               <button onClick={executeDelete} className="w-full bg-red-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest">Sim, Excluir</button>
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-8 backdrop-blur-2xl text-white">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-sm rounded-[3rem] p-10 text-center shadow-3xl">
            ...
            <button onClick={updateProfile} className="w-full bg-amber-500 text-black py-5 rounded-2xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">Salvar Perfil</button>
          </div>
        </div>
      )}
    </div>
