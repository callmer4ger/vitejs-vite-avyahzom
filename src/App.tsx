...
export default function App() {
  ...
  const saveClient = async (e: React.FormEvent) => {
    ...
    let error;
    if (editingClient) {
      const res = await supabase.from('clientes').update(payload).eq('id', editingClient.id);
      error = res.error;
    } else {
  ...
  const stats = useMemo(() => {
    ...
    const needsRecovery = clients.filter(c => differenceInDays(today, parseISO(c.last_visit)) >= 20).length;
    
    const range = { 
      'today': today, 
      'week': subDays(today, 7), 
      'month': subDays(today, 30), 
      '90days': subDays(today, 90), 
      'all': new Date(0) 
  ...
  if (authLoading) return <div className="min-h-screen bg-[#050505] flex items-center justify-center text-amber-500 font-black italic tracking-widest animate-pulse text-xl">BARBER PRO</div>;

  if (!session) return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 text-white">
      <div className="w-full max-w-sm space-y-10">
        <div className="text-center space-y-4">
          <div className="w-20 h-20 bg-amber-500 rounded-[2rem] flex items-center justify-center mx-auto shadow-[0_0_40px_rgba(217,119,6,0.2)]"><Scissors className="text-black w-10 h-10" /></div>
          <h1 className="text-4xl font-black italic tracking-tighter uppercase">BARBER <span className="text-amber-500">PRO</span></h1>
        </div>
        <form onSubmit={(e) => { e.preventDefault(); supabase.auth.signInWithPassword({ email, password }); }} className="bg-zinc-900/50 p-8 rounded-[2.5rem] border border-white/5 space-y-4 shadow-2xl">
          <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="E-mail" className="w-full bg-black border border-white/10 rounded-2xl p-4 outline-none focus:border-amber-500 transition-all" />
          <input required type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Senha" className="w-full bg-black border border-white/10 rounded-2xl p-4 outline-none focus:border-amber-500 transition-all" />
          <button className="w-full bg-amber-500 text-black py-4 rounded-2xl font-black shadow-lg hover:bg-amber-400 active:scale-95 transition-all uppercase">Entrar</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-amber-500/30">
      <header className="sticky top-0 z-40 px-6 h-24 flex items-center justify-between bg-[#050505]/90 backdrop-blur-2xl border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => { resetForm(); setEditingClient(null); setIsModalOpen(true); }} className="bg-amber-500 text-black px-5 py-3 rounded-2xl font-black text-xs flex items-center gap-2 shadow-lg active:scale-95 transition-all"><UserPlus size={18} /> <span className="hidden sm:inline uppercase">Novo Cliente</span></button>
      ...
      <main className="p-6 max-w-2xl mx-auto space-y-8 pb-32">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-zinc-900/50 border border-white/5 p-5 rounded-[2rem] flex flex-col justify-between shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1 group">
            ...
            <input type="text" placeholder="Localizar cliente..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-zinc-900/50 border border-white/5 rounded-2xl pl-14 pr-6 py-4 outline-none focus:border-amber-500/30 transition-all text-sm font-medium" />
          </div>
              ...
              <div key={c.id} className={cn("bg-zinc-900/30 border rounded-[2rem] p-5 flex items-center justify-between transition-all group", isSel ? "border-amber-500 bg-amber-500/5 shadow-[0_0_20px_rgba(217,119,6,0.1)]" : "border-white/5")}>
                <div className="flex items-center gap-4">
                  {isSelectionMode ? (
                    ...
                    <div className="w-12 h-12 bg-gradient-to-br from-zinc-800 to-zinc-900 border border-white/5 rounded-2xl flex items-center justify-center text-amber-500 font-black text-lg shadow-inner">{c.name?.[0]?.toUpperCase()}</div>
                  )}
                  <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <button onClick={() => { setIdsToDelete([c.id]); setIsDeleteModalOpen(true); }} className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500/20 transition-all"><Trash2 size={16} /></button>
                  </div>
                </div>
              </div>
            );
        ...
        {isSelectionMode && selectedClients.length > 0 && (
          <div className="fixed bottom-8 left-0 right-0 px-6 z-50 flex justify-center">
            <button onClick={() => { setIdsToDelete(selectedClients); setIsDeleteModalOpen(true); }} className="max-w-md w-full bg-red-600 text-white py-5 rounded-[2rem] font-black uppercase tracking-widest text-[10px] shadow-[0_10px_30px_rgba(220,38,38,0.3)] animate-bounce">Apagar Selecionados ({selectedClients.length})</button>
          </div>
        )}
      </main>
      ...
      {isRenewModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6 backdrop-blur-2xl overflow-y-auto">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-md rounded-[3rem] p-10 shadow-3xl my-auto">
             ...
             <p className="text-zinc-500 text-[10px] text-center mb-8 uppercase font-bold tracking-widest">Confirme os detalhes do serviço</p>
             <form onSubmit={saveClient} className="space-y-4">
                ...
                <div className="flex gap-4 pt-6">
                  ...
                  <button type="submit" className="flex-1 bg-emerald-500 text-black py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all shadow-lg shadow-emerald-500/10">Zerar Dias</button>
                </div>
             </form>
          </div>
        </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-[60] flex items-center justify-center p-6 backdrop-blur-2xl">
          <div className="bg-zinc-900 border border-red-500/20 w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl animate-in zoom-in duration-200">
             ...
             <p className="text-zinc-500 text-[10px] mb-8 uppercase font-bold tracking-widest leading-relaxed">Essa ação removerá {idsToDelete.length} cliente(s).<br/><span className="text-red-500/50">Não pode ser desfeita.</span></p>
          ...
          <div className="bg-zinc-900 border border-white/5 w-full max-w-md rounded-[3rem] p-10 shadow-2xl my-auto">
            ...
            <form onSubmit={saveClient} className="space-y-5">
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button type="submit" className="flex-1 bg-amber-500 text-black py-5 rounded-[1.5rem] font-black uppercase text-[10px] active:scale-95 shadow-xl shadow-amber-500/10">Salvar</button>
              </div>
            </form>
          </div>
        </div>
