...
export default function App() {
  ...
  const saveClient = async (e: React.FormEvent) => {
    ...
    if (editingClient) {
      // SÓ salva o "anteriormente" se o nome mudou de fato
      if (editingClient.name !== newName) {
  ...
  const { dashboardList, historyList, rankingList } = useMemo(() => {
    ...
    // Ordenar Dashboard por MAIOR ATRASO
  ...
  const revenueChartData = useMemo(() => {
    ...
    const points = interval.map(day => {
      const dailyTotal = allRecords
        .filter(r => isSameDay(parseISO(r.last_visit), day))
